...
const handleReadPermissionWithPagination = async (page, limit) => {
  try {
    const connection = await mysql.createConnection({
      user: process.env.DB_USERNAME,
      password: process.env.DB_PASSWORD,
    ...
    const [totalPagesPermission, total] = await connection.execute(
      `SELECT COUNT(*) AS "total" FROM PERMISSION where PERMISSION_PARENT=0`
    );
    const [permissionParent, parent] = await connection.execute(
      `SELECT a.*, a_group.Count AS "CountRole" FROM PERMISSION a  LEFT OUTER JOIN (SELECT PERMISSION_PARENT, COUNT(*) AS Count FROM PERMISSION GROUP BY PERMISSION_PARENT) a_group ON a.id = a_group.PERMISSION_PARENT where a.PERMISSION_PARENT = 0 LIMIT ${limit} OFFSET ${offset}`
    );
    ...
    if (permissionParent) {
      ...
      let data = {
        ...
        permission: resultPerssion,
      };
      return {
        EM: "get all permission success!",
        EC: 0,
        DT: data,
      };
    } else {
...
module.exports = {
  ...
  handleReadPermissionWithPagination,
};

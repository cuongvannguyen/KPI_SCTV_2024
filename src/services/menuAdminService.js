...
const handleMenuDisplay = async (data) => {
  try {
    ...
    if (!data && !data.groupId) {
      ...
      let query = `SELECT au.* FROM ADMIN_GROUP_MENU agm JOIN MENU_ADMIN au ON agm.MENU_ID = au.id WHERE au.MENU_LEVER = 1 AND au.MENU_STATUS = 1 AND agm.GROUP_ID = ? ORDER BY au.MENU_ORDER;`;
      ...
      if (menuParent && menuParent.length > 0) {
        ...
        for (let i = 0; i < menuParent.length; i++) {
          ...
          const [menuAdminItemL1, item1] = await connection.execute(
            `SELECT au.* FROM ADMIN_GROUP_MENU agm JOIN MENU_ADMIN au ON agm.MENU_ID = au.id WHERE au.MENU_LEVER = 2 AND au.MENU_STATUS = 1 AND agm.GROUP_ID = ? AND au.MENU_PARENT = ? ORDER BY au.MENU_ORDER`,
          ...
          if (menuAdminItemL1 && menuAdminItemL1.length > 0) {
            ...
            for (let i = 0; i < menuAdminItemL1.length; i++) {
              ...
              const [menuAdminItemL2, item2] = await connection.execute(
                `SELECT * FROM MENU_ADMIN where MENU_PARENT=? AND MENU_LEVER = 3 AND MENU_STATUS = 1`,
                [menuItemL1.id]
              );
...
const handleReadMenuAdmin = async () => {
  try {
    ...
    where a.MENU_PARENT = 0 ORDER BY a.MENU_ORDER;`;
    const [menuParent, parent] = await connection.execute(query);
...
const handleReadMenuAdminWithPagination = async (page, limit) => {
  try {
    const connection = await mysql.createConnection({
      user: process.env.DB_USERNAME,
      password: process.env.DB_PASSWORD,
    ...
    const [totalPagesPermission, total] = await connection.execute(
      `SELECT COUNT(*) AS "total" FROM MENU_ADMIN where MENU_PARENT = 0;`
    );

    let query = `SELECT a.*, a_menu.Count AS "CountMenuItem" 
    FROM MENU_ADMIN a  LEFT OUTER JOIN (SELECT MENU_PARENT, COUNT(*) AS Count FROM MENU_ADMIN GROUP BY MENU_PARENT) a_menu ON a.id = a_menu.MENU_PARENT
    where a.MENU_PARENT = 0 ORDER BY a.MENU_ORDER LIMIT ${limit} OFFSET ${offset};`;
    ...
    if (menuParent && menuParent.length > 0) {
      ...
      let totalPages = Math.ceil(totalPagesPermission[0]?.total / limit);
      let data = {
        meta: {
          current: page,
          pageSize: limit,
          pages: totalPages,
          total: totalPagesPermission[0]?.total,
        },
        menuAdmin: resultMenuParent,
      };
...
module.exports = {
  ...
  assignMenuToRole,
  handleMenuDisplay,
  handleReadMenuAdminWithPagination,
};
